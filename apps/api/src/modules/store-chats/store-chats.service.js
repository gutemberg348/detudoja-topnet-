...
function ensureStoreChatPrismaClient() {
  if (!prisma.conversaLoja || !prisma.conversaLojaMensagem) {
    throw new AppError(
...
async function buildCommercialMessage(access, data, scope) {
  ...
  const store = await prisma.loja.findUnique({
    ...
    where: { id: access.conversation.loja_id },
  });
...
async function getConversationAccess(userId, conversationId) {
  ...
  const conversation = await prisma.conversaLoja.findUnique({
    include: {
      cliente: { select: customerSelect },
      loja: {
        include: {
          ...
          usuarios: {
            select: { status: true, usuario_id: true },
          },
        },
      },
    },
...
export async function openStoreConversation(userId, storeIdValue) {
  ...
  const existing = await prisma.conversaLoja.findUnique({
    where: {
  ...
  const conversation = existing ?? await prisma.conversaLoja.create({
    data: {
      cliente_usuario_id: userId,
      loja_id: storeId,
    },
...
export async function listStoreConversations(
  ...
  const conversations = await prisma.conversaLoja.findMany({
      ...
      where: {
        ...
        ...(scope === "customer"
          ? {
              ...
              origem: { in: ["LOJA", "SISTEMA", "ADMIN"] },
            }
          : {
              lido_loja_em: null,
              origem: { in: ["CLIENTE", "SISTEMA"] },
            }),
      },
    });
