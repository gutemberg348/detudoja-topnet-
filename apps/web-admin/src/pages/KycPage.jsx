      ...
      {selected ? (
        <div className="modal-backdrop" onMouseDown={() => !processing && setSelected(null)}>
          <section className="modal modal--wide kyc-review" onMouseDown={(event) => event.stopPropagation()}>
            ...
            {["EM_ANALISE", "REPROVADO"].includes(selected.status) ? <>
              ...
              <div className="modal__actions">{selected.status === "EM_ANALISE" ? <button className="button button--danger" disabled={processing} onClick={() => decide("reject")} type="button"><X size={16} /> Reprovar</button> : null}<button className="button button--primary" disabled={processing} onClick={() => decide("approve")} type="button"><Check size={16} /> {selected.status === "REPROVADO" ? "Reverter e aprovar" : "Aprovar KYC"}</button></div>
