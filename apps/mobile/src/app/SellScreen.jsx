...
function SaleModal({ error, form, isSaving, onChange, onClose, onSubmit, open }) {
  return (
    <Modal animationType="fade" transparent visible={open}>
      <View style={styles.modalBackdrop}>
        <View style={styles.modal}>
...
function SaleDestinationModal({ onClose, onSelectAutonomous, onSelectStore, open, stores }) {
  return (
    <Modal animationType="fade" transparent visible={open}>
      <View style={styles.modalBackdrop}>
        <View style={styles.modal}>
...
function StoreChargeModal({ error, form, isSaving, onChange, onClose, onSubmit, open, store }) {
  return (
    <Modal animationType="fade" transparent visible={open}>
      <View style={styles.modalBackdrop}>
        <View style={styles.modal}>
...
function StoreOrderChatModal({ accessToken, onClose, onMessagesRead, open, order, store }) {
  ...
  const loadMessages = useCallback(async ({ silent = false } = {}) => {
    if (!accessToken || !open || !order?.id || !store?.id) {
      return;
    }
  ...
  async function sendReply() {
    ...
    if (!text || isSendingReply || !accessToken || !store?.id || !order?.id) {
      return;
    }
